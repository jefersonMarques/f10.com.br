...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'ALTER TABLE help_training_step_media DROP CONSTRAINT %I',
...
ALTER TABLE help_training_step_media
  ADD CONSTRAINT help_training_step_media_type_check
  CHECK (media_type IN ('image', 'video', 'caption'));
...
ALTER TABLE help_training_step_media
  ...
  CHECK (
    ...
    OR
    (media_type = 'caption' AND asset_id IS NOT NULL AND source_url IS NULL)
  );
