<script lang="ts">
  ...
  function messageAuthor(message: ChatMessage): string {
    if (message.authorType === "customer") {
      return chat.customerName ?? "Cliente";
    }
  ...
  async function refreshMessages(): Promise<void> {
    ...
    try {
      const response = await fetch(
        `/api/app/chat/${chat.sessionId}/messages`,
...
<svelte:head>
  <title>Chat #{chat.ticketNumber} | F10 Operations</title>
</svelte:head>
...
<div class="mx-auto flex h-[calc(100dvh-78px)] max-w-[1380px] flex-col px-5 py-5 sm:px-8">
  <div class="flex shrink-0 flex-col justify-between gap-4 pb-5 lg:flex-row lg:items-center">
    <div class="flex min-w-0 items-center gap-3">
      <a href="/app/chat" class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-[#606676] shadow-sm transition hover:text-[#000A57]" aria-label="Voltar para conversas">
      ...
      <div class="min-w-0">
        ...
        <p class="mt-1 truncate text-[10px] text-[#898E9B]">
          {chat.organizationName ?? chat.customerEmail ?? "Chat do site"}
        </p>
        {#if chat.aiHandoffReason}
          <p class="mt-1 line-clamp-1 text-[9px] text-[#9A6464]">{chat.aiHandoffReason}</p>
        {/if}
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2">
      {#if chat.contextUrl}
        <a href={chat.contextUrl} target="_blank" rel="noopener noreferrer" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-3 text-[10px] font-semibold text-[#666C7B]">
          Página de origem
          <ExternalLink size={13} aria-hidden="true" />
        </a>
      {/if}
      <a href={`/app/tickets/${chat.ticketId}`} class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[#000A57] px-3 text-[10px] font-semibold text-white">
        Abrir ticket
  ...
  <section class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[24px] border border-[#E2E5ED] bg-white shadow-[0_14px_40px_rgba(1,13,40,0.05)]">
    <header class="flex shrink-0 items-center justify-between border-b border-[#EEF0F5] px-5 py-3.5">
      ...
      <span class="text-[9px] text-[#989DAA]">
        {chat.assignedUserName ? `Atendendo: ${chat.assignedUserName}` : chat.aiState === "active" ? "Agente IA atendendo" : "Aguardando atendente"}
      </span>
    </header>
          ...
          <div class={`flex ${message.authorType === "customer" ? "justify-start" : "justify-end"}`}>
            <article class={`max-w-[82%] rounded-2xl px-4 py-3 shadow-sm ${message.authorType === "customer" ? "rounded-bl-md border border-[#E0E3EA] bg-white text-[#565C6B]" : message.authorType === "system" ? "rounded-br-md border border-[#D9D4F5] bg-[#F2F0FF] text-[#403878]" : "rounded-br-md bg-[#000A57] text-white"}`}>
              {#if message.authorType === "system"}
                <div class="mb-2 flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-[0.08em] text-[#6255A8]"><Bot size={12} aria-hidden="true" />Agente IA</div>
              {/if}
              <p class="whitespace-pre-wrap text-[12px] leading-5">{message.body}</p>
              <div class={`mt-2 flex items-center gap-2 text-[8px] ${message.authorType === "customer" ? "text-[#9A9FAC]" : message.authorType === "system" ? "text-[#8178B5]" : "text-white/60"}`}>
      ...
      <footer class="shrink-0 border-t border-[#E6E8EE] bg-white p-4 sm:p-5">
        <div class="mx-auto max-w-[840px]">
          {#if chat.aiState === "active"}
            <div class="mb-3 flex items-center gap-2 rounded-xl bg-[#F4F2FF] px-3 py-2 text-[9px] font-medium text-[#6255A8]"><Bot size={14} aria-hidden="true" />Ao enviar uma resposta, você assume a conversa e o agente de IA é interrompido.</div>
          {/if}
          {#if errorMessage}
          ...
          <form on:submit|preventDefault={() => void sendMessage()} class="flex items-end gap-3">
            <label class="min-w-0 flex-1">
              ...
              <textarea bind:value={messageBody} maxlength="4000" rows="2" placeholder="Escreva uma mensagem..." class="max-h-32 min-h-[52px] w-full resize-none rounded-2xl border border-[#DDE1EA] px-4 py-3 text-[12px] leading-5 outline-none focus:border-[#000A57] focus:ring-2 focus:ring-[#000A57]/10"></textarea>
            </label>
            <button type="submit" disabled={sending || !messageBody.trim()} class="inline-flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-2xl bg-[#000A57] text-white transition hover:bg-[#111B71] disabled:bg-[#D6D9E2]" aria-label="Enviar mensagem">
