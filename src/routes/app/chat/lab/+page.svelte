  ...
  <section class="rounded-[22px] border border-[#E2E5ED] bg-white p-5 shadow-[0_10px_32px_rgba(1,13,40,0.04)] sm:p-6">
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <p class="application-text-caption mt-1 leading-5 text-[#858B99]">O laboratório usa conhecimento público e a camada privada destinada à IA, sempre derivados da última publicação.</p>
      </div>
    </div>
    ...
    {#if data.canAsk}
      <form method="POST" action="?/ask" class="mt-5">
        ...
        <div class="mt-3 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div class="application-text-meta flex items-start gap-2 leading-5 text-[#8A909E]">
      ...
      <div class="px-5 py-5 sm:px-6">
        ...
        {#if form.result.escalationReason}
          <div class="mt-5 rounded-2xl border border-[#F0D0D0] bg-[#FFF8F8] px-4 py-3">
            <p class="application-text-meta font-bold uppercase tracking-[0.08em] text-[#A34242]">Motivo do escalonamento</p>
        ...
        <div class="mt-6">
          ...
          {#if form.result.sources.length === 0}
            <p class="application-text-caption mt-3 rounded-xl bg-[#F7F8FB] px-4 py-3 text-[#858B99]">Nenhuma fonte publicada correspondeu à pergunta.</p>
          {:else}
              ...
              {#each form.result.sources as source}
                <a href={`/app/help/content/${source.contentId}`} class="group rounded-2xl border border-[#E2E5ED] p-4 transition hover:border-[#C9CEE0] hover:bg-[#FAFAFC]">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <span class="application-text-meta font-bold uppercase tracking-[0.08em] text-[#EA6D0B]">Fonte {source.rank}</span>
                      <strong class="mt-1 block truncate text-[11px] font-semibold text-[#303645]">{source.title}</strong>
                      <span class="application-text-meta mt-1 block truncate text-[#9A9FAB]">/{source.slug}</span>
                    </div>
  ...
  <section class="mt-5 overflow-hidden rounded-[22px] border border-[#E2E5ED] bg-white">
    <header class="flex items-center gap-3 border-b border-[#EEF0F5] px-5 py-4 sm:px-6">
      ...
      <div>
        ...
        <p class="application-text-caption mt-1 text-[#8A909E]">Histórico persistido para auditoria e melhoria do agente.</p>
      </div>
    </header>
        ...
        {#each data.recentRuns as run}
          <div class="px-5 py-4 sm:px-6">
            <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
              ...
              <div class="application-text-meta shrink-0 text-[#A0A5B0] sm:text-right">
