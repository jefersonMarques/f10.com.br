...
<ApplicationContent width="standard">
  ...
  <section class="rounded-[24px] border border-[#E2E5ED] bg-white p-5 sm:p-7">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="mt-1 max-w-[760px] text-[11px] leading-5 text-[#858B99]">Alternativa ao fluxo manual de prompt + ZIP. O servidor extrai o áudio, cria uma transcrição temporal, estrutura o artigo e captura os screenshots localmente nos tempos definidos pela IA.</p>
        </div>
      </div>
    ...
    <div class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <div class="rounded-2xl border border-[#E2E5ED] bg-[#FAFBFD] p-4"><span class="application-text-meta uppercase tracking-[0.06em] text-[#969BA8]">OpenAI</span><strong class={`mt-2 block text-[11px] ${data.runtime.openAi ? "text-[#2F7045]" : "text-[#9B3C3C]"}`}>{data.runtime.openAi ? "Configurada" : "Pendente"}</strong><small class="mt-1 block text-[9px] leading-4 text-[#858B99]">Estruturação textual: {data.model}</small></div>
    ...
    <div class="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      <div class={`rounded-2xl border p-4 ${mp4Ready ? "border-[#CFE4D6] bg-[#F5FBF7]" : "border-[#F0D8C3] bg-[#FFF9F3]"}`}><strong class="text-[11px] text-[#343B4B]">Arquivo .mp4</strong><p class="mt-2 text-[10px] leading-5 text-[#727887]">Não depende de yt-dlp. O MP4 enviado é processado localmente, armazenado no F10 e vinculado automaticamente como vídeo principal do artigo.</p></div>
    </div>

    <div class="mt-5 rounded-2xl border border-[#D8DDF4] bg-[#F8F9FF] p-4">
      <div class="flex items-start gap-3"><Bot size={17} class="mt-0.5 shrink-0 text-[#000A57]"/><p class="text-[10px] leading-5 text-[#5D6475]">A OpenAI não recebe o vídeo bruto nem os screenshots. O áudio gera a transcrição com timestamps; o modelo textual define uma janela e se a captura deve representar o estado antes ou depois da ação. O FFmpeg extrai seis candidatos nessa janela e o F10 compara os frames localmente para escolher o estado mais estável.</p></div>
    </div>
