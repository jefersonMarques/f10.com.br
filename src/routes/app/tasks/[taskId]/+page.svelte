...
<ApplicationContent width="standard">
  ...
  <section class="mb-4 rounded-[22px] border border-[#E2E5ED] bg-white px-5 py-4 sm:px-6">
    <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
      <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2"><span class="application-text-caption rounded-full bg-[#EEF0FF] px-3 py-1.5 font-bold text-[#000A57]">{data.details.task.statusName}</span><span class="application-text-caption rounded-full bg-[#F3F4F7] px-3 py-1.5 font-semibold text-[#737989]">{priorityLabels[data.details.task.priority]}</span>{#if data.googleLink}<span class="application-text-caption inline-flex items-center gap-1 rounded-full bg-[#EEF7F1] px-3 py-1.5 font-semibold text-[#2F7045]"><CalendarCheck2 size={12}/>Google Calendar</span>{/if}{#if data.googleLink?.googleMeetEnabled}<span class="application-text-caption inline-flex items-center gap-1 rounded-full bg-[#EEF3FF] px-3 py-1.5 font-semibold text-[#214A9A]"><Video size={12}/>Google Meet</span>{/if}</div>
        ...
        <p class="application-text-caption mt-1 text-[#7C8291]">Projeto: {data.details.task.projectName}</p>
      </div>
      {#if data.canUpdate}
            ...
            {#if data.googleCalendar.connected}
              <div class="sm:col-span-2 rounded-2xl border border-[#DDE7E1] bg-[#F8FBF9] p-4">
                ...
                <div class="flex flex-wrap items-start justify-between gap-3">
                  <label class="flex cursor-pointer items-start gap-3"><input type="checkbox" name="syncGoogle" value="true" bind:checked={syncGoogle} class="mt-1"/><span><strong class="block text-[11px] font-semibold text-[#2E3B33]">Sincronizar com Google Calendar</strong><span class="application-text-meta mt-1 block leading-4 text-[#78827B]">Usa sua conta {data.googleCalendar.googleEmail}. Alterações no F10 atualizam o mesmo evento e os convidados.</span></span></label>
                  {#if data.googleLink?.googleHtmlLink}<a href={data.googleLink.googleHtmlLink} target="_blank" rel="noreferrer" class="application-text-meta inline-flex items-center gap-1 font-semibold text-[#000A57]">Abrir evento <ExternalLink size={12}/></a>{/if}
                </div>
                {#if syncGoogle}
                  <div class="mt-4 space-y-3 rounded-xl border border-[#E1E8E3] bg-white p-3">
                    <label class="application-text-caption flex items-center gap-2 font-medium text-[#535E57]"><input type="checkbox" name="googleAllDay" value="true" bind:checked={googleAllDay}/>Evento de dia inteiro</label>
                    {#if !googleAllDay}
                      <div class="grid grid-cols-2 gap-3"><label><span class="application-text-meta mb-1 block font-semibold text-[#6E776F]">Início</span><input type="time" name="googleStartTime" bind:value={googleStartTime} required class="h-10 w-full rounded-lg border border-[#DDE1EA] px-2 text-[11px]"/></label><label><span class="application-text-meta mb-1 block font-semibold text-[#6E776F]">Fim</span><input type="time" name="googleEndTime" bind:value={googleEndTime} required class="h-10 w-full rounded-lg border border-[#DDE1EA] px-2 text-[11px]"/></label></div>
                    {:else}
                    ...
                    {#if data.googleLink?.googleMeetEnabled}
                      ...
                      <div class="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#D8E2F6] bg-[#F5F8FF] px-3 py-3">
                        <span class="application-text-meta inline-flex items-center gap-2 font-semibold text-[#214A9A]"><Video size={14}/>Google Meet ativo</span>
                        {#if data.googleLink.googleMeetUrl}<a href={data.googleLink.googleMeetUrl} target="_blank" rel="noreferrer" class="application-text-meta inline-flex min-h-8 items-center gap-1 rounded-lg bg-[#214A9A] px-3 font-semibold text-white">Entrar na reunião <ExternalLink size={11}/></a>{:else}<span class="application-text-meta text-[#77839A]">O Google ainda está finalizando o link da reunião.</span>{/if}
                      </div>
                    {:else}
                      <label class="flex cursor-pointer items-start gap-2 rounded-xl border border-[#DFE5F1] bg-[#FAFBFE] px-3 py-3"><input type="checkbox" name="googleMeet" value="true" bind:checked={googleMeet} class="mt-0.5"/><span><strong class="application-text-meta block font-semibold text-[#35445F]">Gerar Google Meet</strong><span class="application-text-meta mt-0.5 block leading-4 text-[#7D8797]">Cria um link exclusivo de reunião dentro deste evento.</span></span></label>
                    {/if}
                    ...
                    <p class="application-text-meta text-[#929A94]">Fuso: {googleTimeZone}. Desmarcar a sincronização remove do Google o evento criado pelo F10.</p>
                    {#if data.googleLink?.lastSyncError}<p class="application-text-meta rounded-lg bg-[#FFF4E9] px-2 py-2 font-medium text-[#A9510D]">A última sincronização apresentou erro. Salve novamente para tentar atualizar o evento.</p>{/if}
                  </div>
                {/if}
              </div>
            {:else}
              <div class="sm:col-span-2 rounded-2xl border border-[#E3E6ED] bg-[#FAFAFC] p-4"><div class="flex items-center gap-2"><CalendarCheck2 size={16} class="text-[#000A57]"/><strong class="text-[11px] text-[#3D4453]">Google Calendar</strong></div><p class="application-text-meta mt-1 leading-4 text-[#858B98]">Conecte sua conta para adicionar esta tarefa à sua agenda.</p><a href="/app/tasks/calendar/google/connect" class="application-text-meta mt-3 inline-flex min-h-9 items-center rounded-lg border border-[#D3D8E3] bg-white px-3 font-semibold text-[#000A57]">Conectar Google Calendar</a></div>
            {/if}
