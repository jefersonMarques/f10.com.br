<script lang="ts">
  ...
  const priorityClasses: Record<string, string> = {
    low: "bg-app-muted text-[#667085]",
    normal: "bg-app-info-bg text-[#000A57]",
    high: "bg-app-warning-bg text-[#A9510D]",
    urgent: "bg-app-danger-bg text-[#A52A2A]",
  };
...
<ApplicationContent width="full">
  {#if form?.message}
    <div class={`mb-3 flex items-start gap-3 rounded-2xl border px-4 py-3 text-[12px] font-medium ${form.success ? "border-[#B9E6C9] bg-app-success-bg text-[#176B35]" : "border-[#F0C8C8] bg-app-danger-bg text-[#9B2C2C]"}`}>
  ...
  <section class="overflow-hidden rounded-[22px] border border-[#E2E5ED] bg-app-surface">
    <div class="flex flex-col gap-3 border-b border-[#EEF0F5] px-4 py-4 sm:px-5 lg:flex-row lg:items-center lg:justify-between">
      <nav class="flex min-w-0 gap-1 overflow-x-auto" aria-label="Escopo das tarefas">
        <a href="/app/tasks" class={`shrink-0 rounded-xl px-4 py-2.5 text-[12px] font-semibold transition ${data.mode === "mine" ? "bg-app-primary text-white" : "text-[#656B7B] hover:bg-app-muted hover:text-[#000A57]"}`}>Minhas tarefas</a>
        {#each data.projects as project}
          <a href={projectHref(project.id)} class={`shrink-0 rounded-xl px-4 py-2.5 text-[12px] font-semibold transition ${project.id === data.selectedProjectId ? "bg-app-info-bg text-[#000A57]" : "text-[#656B7B] hover:bg-app-muted hover:text-[#000A57]"}`}>{project.name}</a>
        {/each}
      </nav>
      <div class="flex flex-wrap items-center gap-2">
        <label class="relative block min-w-[220px] lg:w-[300px]"><Search size={15} class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#9499A5]"/><input bind:value={searchTerm} placeholder="Buscar tarefas" class="h-10 w-full rounded-xl border border-[#DDE1EA] bg-app-subtle pl-9 pr-3 text-[12px] outline-none focus:border-[#000A57] focus:bg-app-surface"/></label>
        {#if data.canManage}<button type="button" on:click={() => (newProjectOpen = true)} class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#DDE1EA] bg-app-surface px-3 text-[11px] font-semibold text-[#000A57] transition hover:border-[#B9C0CF]"><Plus size={15}/>Novo projeto</button>{/if}
      </div>
    </div>
    ...
    {#if data.mode === "mine"}
      <div class="px-4 py-5 sm:px-5">
        <div class="flex items-start justify-between gap-4">
          ...
          <span class="application-text-caption rounded-full bg-app-muted px-3 py-1.5 font-semibold text-[#727887]">{data.myTasks.length} no total</span>
        </div>

        {#if filteredMyTasks.length === 0}
          <div class="mt-6 rounded-2xl border border-dashed border-[#D6DAE3] bg-app-subtle px-5 py-12 text-center"><CheckCircle2 size={30} class="mx-auto text-[#A9AFBC]"/><h3 class="mt-4 text-[14px] font-semibold text-[#444A59]">Nada por aqui</h3><p class="mt-1 text-[11px] text-[#8A909E]">Você não tem tarefas atribuídas que correspondam à busca.</p></div>
        {:else}
          <div class="mt-5 space-y-6">
            {#each [
              ...
              {#if group.tasks.length > 0}
                <section>
                  <header class="mb-2 flex items-center gap-2 px-2"><span class={`h-2 w-2 rounded-full ${group.key === "overdue" ? "bg-red-600" : group.key === "completed" ? "bg-emerald-600" : "bg-app-accent"}`}></span><h3 class="text-[11px] font-bold uppercase tracking-[0.08em] text-[#6E7483]">{group.title}</h3><span class="application-text-caption text-[#A0A5B0]">{group.tasks.length}</span></header>
                  <div class="overflow-hidden rounded-xl border border-[#E7E9EF]">
                    {#each group.tasks as task}
                      <a href={taskHref(task.id)} class="grid min-h-12 grid-cols-[minmax(0,1fr)] items-center gap-2 border-b border-[#F0F1F4] px-3 py-2.5 transition last:border-b-0 hover:bg-app-subtle md:grid-cols-[minmax(260px,1.8fr)_minmax(130px,0.8fr)_110px_100px_minmax(130px,0.9fr)]">
                        <div class="flex min-w-0 items-center gap-3"><span class={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${task.statusClosed ? "border-[#A9D7B7] bg-app-success-bg text-[#287645]" : "border-[#D5D9E2] text-transparent"}`}>{#if task.statusClosed}<Check size={13}/>{/if}</span><span class="min-w-0"><strong class={`block truncate text-[12px] font-semibold ${task.statusClosed ? "text-[#8B909C] line-through" : "text-[#2D3342]"}`}>{task.title}</strong>{#if originsFor(task.id).length > 0}<span class="application-text-caption mt-0.5 inline-flex items-center gap-1 font-medium text-[#000A57]"><Ticket size={11}/>Ticket #{originsFor(task.id)[0].ticketNumber}</span>{/if}</span></div>
    ...
    {:else if data.board}
      <div class="px-4 py-5 sm:px-5">
        ...
        <div class="mt-5 flex flex-wrap items-center gap-2 rounded-2xl border border-[#E8EAF0] bg-app-subtle p-3">
          ...
          <select bind:value={filterState} class="application-text-caption h-9 rounded-lg border border-[#DDE1EA] bg-app-surface px-2"><option value="open">Abertas</option><option value="all">Todas</option><option value="closed">Concluídas</option></select>
          <select bind:value={filterAssignee} class="application-text-caption h-9 rounded-lg border border-[#DDE1EA] bg-app-surface px-2"><option value="all">Todos responsáveis</option>{#each data.members as member}<option value={member.id}>{member.name}</option>{/each}</select>
          <select bind:value={filterPriority} class="application-text-caption h-9 rounded-lg border border-[#DDE1EA] bg-app-surface px-2"><option value="all">Todas prioridades</option><option value="urgent">Urgente</option><option value="high">Alta</option><option value="normal">Normal</option><option value="low">Baixa</option></select>
          <select bind:value={filterOrigin} class="application-text-caption h-9 rounded-lg border border-[#DDE1EA] bg-app-surface px-2"><option value="all">Todas origens</option><option value="ticket">Vindas de ticket</option><option value="standalone">Sem ticket</option></select>
          <span class="application-text-caption ml-auto rounded-full bg-app-surface px-3 py-1.5 font-semibold text-[#777D8D] shadow-sm">{filteredProjectTasks.length} tarefa(s)</span>
        </div>

        {#if data.canCreate}
          <form use:enhance method="POST" action="?/createTask" class={`mt-4 grid gap-2 rounded-2xl border border-dashed border-[#CDD2DD] bg-app-surface p-3 ${data.canAssign ? "md:grid-cols-[minmax(220px,1.6fr)_150px_135px_150px_auto]" : "md:grid-cols-[minmax(220px,1.6fr)_135px_150px_auto]"}`}>
            <input type="hidden" name="projectId" value={data.board.project.id}/>
            <input name="title" required minlength="3" maxlength="180" placeholder="+ Adicionar tarefa" class="h-10 min-w-0 rounded-xl border border-transparent bg-app-subtle px-3 text-[12px] font-medium outline-none focus:border-[#000A57] focus:bg-app-surface"/>
            {#if data.canAssign}<select name="assigneeId" class="application-text-caption h-10 rounded-xl border border-[#E1E4EA] bg-app-surface px-2"><option value="">Atribuir a mim</option>{#each data.members as member}<option value={member.id}>{member.name}</option>{/each}</select>{/if}
            <input name="dueOn" type="date" class="application-text-caption h-10 rounded-xl border border-[#E1E4EA] bg-app-surface px-2"/>
            <select name="priority" class="application-text-caption h-10 rounded-xl border border-[#E1E4EA] bg-app-surface px-2"><option value="normal">Normal</option><option value="low">Baixa</option><option value="high">Alta</option><option value="urgent">Urgente</option></select>
            <button type="submit" class="h-10 rounded-xl bg-app-primary px-4 text-[11px] font-semibold text-white">Adicionar</button>
          </form>
        {/if}

        {#if data.view === "list"}
          <div class="mt-5 overflow-hidden rounded-2xl border border-[#E3E6ED]">
            <div class="application-text-meta hidden grid-cols-[minmax(300px,1.8fr)_140px_130px_110px_150px_40px] gap-2 border-b border-[#E8EAF0] bg-app-subtle px-4 py-3 font-bold uppercase tracking-[0.08em] text-[#8A909E] md:grid"><span>Tarefa</span><span>Responsável</span><span>Prazo</span><span>Prioridade</span><span>Status</span><span></span></div>
            {#each filteredProjectTasks as task}
              <a href={taskHref(task.id)} class="grid min-h-14 grid-cols-[minmax(0,1fr)] items-center gap-2 border-b border-[#EEF0F4] px-4 py-3 transition last:border-b-0 hover:bg-app-subtle md:grid-cols-[minmax(300px,1.8fr)_140px_130px_110px_150px_40px]">
          ...
          <div class="mt-4 overflow-x-auto pb-3">
            <div class="grid min-w-[940px] gap-4" style={`grid-template-columns: repeat(${Math.max(data.board.statuses.length, 1)}, minmax(290px, 1fr));`}>
              {#each data.board.statuses as status}
                <section
                  class={`min-h-[260px] rounded-[20px] border p-3 transition ${dragOverStatusId === status.id ? "border-[#000A57] bg-app-info-bg" : "border-[#E0E3EB] bg-app-subtle"}`}
                  role="group"
                ...
                >
                  <header class="flex items-center justify-between gap-3 px-1 pb-3"><div class="flex items-center gap-2"><span class={`h-2.5 w-2.5 rounded-full ${status.isClosed ? "bg-emerald-600" : "bg-app-accent"}`}></span><h3 class="text-[12px] font-semibold text-[#3A4050]">{status.name}</h3></div><span class="application-text-meta rounded-full bg-app-surface px-2 py-1 font-semibold text-[#858B99] shadow-sm">{tasksForStatus(status.id).length}</span></header>
                  <div class="space-y-3">
                    {#each tasksForStatus(status.id) as task (task.id)}
                      <article
                        ...
                        on:dragend={endDrag}
                        class={`group rounded-2xl border bg-app-surface p-4 shadow-[0_5px_18px_rgba(1,13,40,0.04)] transition ${draggingTaskId === task.id ? "border-[#000A57] opacity-60" : "border-[#E2E5EC] hover:border-[#C9CEDA]"}`}
                      >
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-2">{#if task.dueOn}<span class={`application-text-meta inline-flex items-center gap-1.5 rounded-lg bg-app-muted px-2 py-1.5 font-medium ${dueClass(task.dueOn, status.isClosed)}`}><CalendarDays size={12}/>{formatDueDate(task.dueOn)}</span>{/if}{#if task.assignees[0]}<span class="application-text-meta rounded-lg bg-app-info-bg px-2 py-1.5 font-medium text-[#000A57]">{task.assignees[0].name}</span>{/if}{#if originsFor(task.id).length > 0}<span class="application-text-meta inline-flex items-center gap-1 rounded-lg bg-app-warning-bg px-2 py-1.5 font-medium text-[#A9510D]"><Ticket size={11}/>#{originsFor(task.id)[0].ticketNumber}</span>{/if}</div>
                      </article>
                    {/each}
                    {#if tasksForStatus(status.id).length === 0}<div class="application-text-caption rounded-2xl border border-dashed border-[#D6DAE3] bg-app-surface px-4 py-10 text-center text-[#9A9FAC]">Solte uma tarefa aqui</div>{/if}
                  </div>
                </section>
              {/each}
            </div>
...
{#if newProjectOpen}
  <div class="fixed inset-0 z-[80] flex items-center justify-center bg-app-overlay/35 p-4" role="presentation">
    ...
    <div class="relative z-10 w-full max-w-[620px] rounded-[24px] border border-[#E1E4EC] bg-app-surface p-5 shadow-2xl sm:p-6" role="dialog" aria-modal="true" aria-label="Novo projeto">
      <div class="flex items-start justify-between gap-4"><div><h2 class="text-[18px] font-semibold text-[#202637]">Novo projeto</h2><p class="mt-1 text-[11px] text-[#858A98]">Crie o espaço de trabalho e escolha os integrantes iniciais.</p></div><button type="button" on:click={() => (newProjectOpen = false)} class="flex h-9 w-9 items-center justify-center rounded-xl text-[#7F8594] hover:bg-app-muted" aria-label="Fechar"><X size={17}/></button></div>
      <form method="POST" action="?/createProject" class="mt-5 space-y-4">
        ...
        {#if data.activeUsers.length > 0}<fieldset><legend class="text-[11px] font-semibold text-[#4A5060]">Integrantes</legend><div class="mt-2 grid max-h-48 gap-2 overflow-y-auto rounded-xl border border-[#E7E9EF] p-2 sm:grid-cols-2">{#each data.activeUsers as user}<label class="flex items-center gap-2 rounded-lg px-2 py-2 text-[11px] text-[#555B69] hover:bg-app-muted"><input type="checkbox" name="memberIds" value={user.id}/><span class="truncate">{user.name}</span></label>{/each}</div></fieldset>{/if}
        <button type="submit" class="min-h-11 w-full rounded-xl bg-app-primary px-5 text-[12px] font-semibold text-white">Criar projeto</button>
      </form>
    </div>
  </div>
{/if}
...
{#if data.selectedTask}
  <div class="fixed inset-0 z-[75] bg-app-overlay/25" role="presentation">
    <a href={closeTaskHref()} class="absolute inset-0" aria-label="Fechar detalhes da tarefa"></a>
    <aside class="absolute inset-y-0 right-0 z-10 w-full max-w-[620px] overflow-y-auto border-l border-[#DDE1EA] bg-app-surface shadow-[-18px_0_55px_rgba(1,13,40,0.16)]" aria-label="Detalhes da tarefa">
      <div class="sticky top-0 z-20 flex items-center justify-between border-b border-[#EEF0F5] bg-app-surface px-5 py-4 backdrop-blur"><div class="flex min-w-0 items-center gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-app-info-bg text-[#000A57]"><CheckCircle2 size={17}/></span><div class="min-w-0"><span class="application-text-meta block font-bold uppercase tracking-[0.08em] text-[#EA6D0B]">{data.selectedTask.details.task.projectName}</span><strong class="block truncate text-[13px] text-[#2D3342]">Detalhes da tarefa</strong></div></div><a href={closeTaskHref()} class="flex h-9 w-9 items-center justify-center rounded-xl text-[#747A89] hover:bg-app-muted" aria-label="Fechar"><X size={18}/></a></div>

      <div class="space-y-6 p-5 sm:p-6">
        {#if data.selectedTask.ticketOrigins.length > 0}
          <div class="rounded-2xl border border-[#F0D6BD] bg-app-warning-bg p-4"><div class="application-text-caption flex items-center gap-2 font-bold uppercase tracking-[0.08em] text-[#A9510D]"><Ticket size={14}/>Origem do suporte</div>{#each data.selectedTask.ticketOrigins as origin}<a href={`/app/tickets/${origin.id}`} class="mt-2 flex items-center justify-between gap-3 rounded-xl bg-app-surface px-3 py-2.5 text-[11px] font-semibold text-[#000A57] shadow-sm"><span class="truncate">Ticket #{origin.ticketNumber} · {origin.subject}</span><ChevronRight size={14}/></a>{/each}</div>
        {/if}
        ...
        <section class="grid gap-4 rounded-2xl border border-[#E7E9EF] bg-app-subtle p-4 sm:grid-cols-2">
          ...
          {#if data.canAssign}<form use:enhance method="POST" action="?/assignTask" class="sm:col-span-2"><input type="hidden" name="taskId" value={data.selectedTask.details.task.id}/><div class="flex gap-2"><select name="assigneeId" required class="h-10 min-w-0 flex-1 rounded-xl border border-[#DDE1EA] bg-app-surface px-3 text-[11px]">{#each data.selectedTask.details.projectMembers as member}<option value={member.id} selected={member.id === data.selectedTask.details.assignees[0]?.userId}>{member.name}</option>{/each}</select><button type="submit" class="application-text-caption h-10 rounded-xl border border-[#CCD1DD] bg-app-surface px-3 font-semibold text-[#000A57]">Atribuir</button></div></form>{/if}
        </section>
