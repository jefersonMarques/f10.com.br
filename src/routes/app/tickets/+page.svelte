<script lang="ts">
  ...
  async function dropTicket(event: DragEvent, stageId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) window.alert("Não foi possível mover o ticket para esta coluna.");
  ...
  async function openCard(ticketId: string): Promise<void> {
    ...
    try {
      ...
      if (card.workflowContext?.areaWorkflowId && data.workflowBoard.areaWorkflows.some((workflow) => workflow.id === card?.workflowContext?.areaWorkflowId)) {
  ...
  async function postCardAction(action: string, body: FormData): Promise<boolean> {
  ...
  async function moveCard(): Promise<void> {
    ...
    await postCardAction("moveTicketLocation", body);
  }
  ...
  <section class="mt-6 overflow-hidden rounded-[22px] border border-[#E2E5ED] bg-white">
    ...
    {#if view === "board"}
      <div class="flex flex-col gap-3 border-b border-[#EEF0F5] p-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-2">{#if activeWorkflow?.kind === "area"}<Boxes size={15} class="text-[#EA6D0B]"/>{:else}<GitBranch size={15} class="text-[#000A57]"/>{/if}<div><strong class="block text-[10px] text-[#343B4B]">{activeWorkflow?.kind === "area" ? `${activeWorkflow.areaName} · processo interno` : "Kanban global"}</strong><span class="text-[8px] text-[#858B99]">{activeWorkflow?.kind === "area" ? "Somente as colunas internas desta área." : "Áreas aparecem como colunas sem expor o processo interno."}</span></div></div>
      ...
      <div class="overflow-x-auto p-4">
        <div class="grid min-w-max gap-4" style={`grid-template-columns: repeat(${Math.max(stages.length, 1)}, minmax(280px, 300px));`}>
          {#each stages as stage}
            <section class={`min-h-[390px] rounded-[16px] border p-3 ${stage.stageType === "area_gateway" ? "border-[#E8C49F] bg-[#FFF8F1]" : "border-[#E0E3EB] bg-[#F5F6F8]"}`} on:dragover|preventDefault on:drop={(event) => void dropTicket(event, stage.id)}>
              <header class="mb-3 flex items-start justify-between gap-3"><div><strong class="text-[11px] text-[#303746]">{stage.name}</strong>{#if stage.stageType === "area_gateway"}<span class="mt-1 flex items-center gap-1 text-[8px] font-semibold text-[#A76225]"><Boxes size={10}/>Área · {stage.linkedAreaName}</span>{/if}</div><span class="rounded-full bg-white px-2 py-1 text-[8px] text-[#777D8C] shadow-sm">{ticketsForStage(stage.id).length}</span></header>
      ...
      <div class="min-w-0 bg-white">
        ...
        <div class="space-y-7 px-5 py-6 sm:px-7">
          ...
          <section>
            ...
            {#if data.canReply}
              <form on:submit={addCardComment} class="mt-3 flex gap-2">
                <textarea name="body" required maxlength="10000" rows="2" placeholder="Escrever um comentário interno..." class="min-h-[54px] flex-1 resize-y rounded-xl border border-[#DCE0E6] bg-white px-3 py-2 text-[9px] leading-4 outline-none focus:border-[#000A57]"></textarea>
                <button class="self-end rounded-lg bg-[#000A57] px-3 py-2 text-[8px] font-semibold text-white">Comentar</button>
              </form>
            {/if}
            <div class="mt-3 space-y-3">
      ...
      <aside class="border-l border-[#E0E3E8] bg-[#F5F6F8] p-5 sm:p-6">
        <div class="space-y-5">
          <section class="rounded-xl border border-[#DDE1E7] bg-white p-4">
            ...
            <p class="mt-1 text-[8px] leading-4 text-[#858B99]">Troque diretamente o contexto do ticket. Áreas só aparecem aqui quando você pode ver o processo interno.</p>
