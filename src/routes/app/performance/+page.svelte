  ...
  <section class="mt-6 overflow-hidden rounded-[24px] border border-[#DCE7DF] bg-white">
    <div class="flex flex-col justify-between gap-4 border-b border-[#E8F0EA] px-5 py-5 sm:px-6 lg:flex-row lg:items-start">
      <div class="flex items-start gap-3">
        ...
        <div>
          ...
          <p class="mt-1 max-w-[760px] text-[10px] leading-5 text-[#788080]">Volume de novos tickets pelo dia e hora em que o cliente procurou a F10. A intensidade considera somente entradas dentro do horário de atendimento.</p>
        </div>
      </div>
      <div class="shrink-0 text-left lg:text-right">
        <span class="block text-[9px] font-bold uppercase tracking-[0.08em] text-[#8B9290]">Período analisado</span>
    ...
    <div class="grid gap-3 border-b border-[#E8F0EA] bg-[#FBFDFC] px-5 py-4 sm:grid-cols-3 sm:px-6">
      <article class="rounded-2xl border border-[#DDE9E0] bg-white px-4 py-3">
        <span class="text-[8px] font-bold uppercase tracking-[0.08em] text-[#8B9290]">Maior pico</span>
        ...
        <span class="mt-1 block text-[9px] text-[#87908A]">{traffic.peakCell ? `${traffic.peakCell.count} atendimento(s)` : "Ainda não há entradas suficientes"}</span>
      </article>
      <article class="rounded-2xl border border-[#DDE9E0] bg-white px-4 py-3">
        <span class="text-[8px] font-bold uppercase tracking-[0.08em] text-[#8B9290]">Dia mais movimentado</span>
        <strong class="mt-1.5 block text-[13px] text-[#245E39]">{traffic.peakDay?.dayLabel ?? "Sem amostra"}</strong>
        <span class="mt-1 block text-[9px] text-[#87908A]">{traffic.peakDay ? `${traffic.peakDay.count} atendimento(s) no expediente` : "Ainda não há entradas suficientes"}</span>
      </article>
      <article class="rounded-2xl border border-[#DDE9E0] bg-white px-4 py-3">
        <span class="text-[8px] font-bold uppercase tracking-[0.08em] text-[#8B9290]">Faixa mais movimentada</span>
        <strong class="mt-1.5 block text-[13px] text-[#245E39]">{peakWindowLabel()}</strong>
        <span class="mt-1 block text-[9px] text-[#87908A]">{traffic.peakWindow ? `${traffic.peakWindow.count} entradas · ${percent(traffic.peakWindow.percent)} do expediente` : "Ainda não há entradas suficientes"}</span>
      </article>
    </div>
    ...
    <div class="px-4 py-5 sm:px-6">
      <div class="overflow-x-auto pb-2">
        <div class="min-w-[760px]">
          <div class="grid items-end gap-2" style={`grid-template-columns: 118px repeat(${traffic.hours.length}, minmax(50px, 1fr));`}>
            <div class="pb-2 text-[8px] font-bold uppercase tracking-[0.08em] text-[#969D99]">Dia / hora</div>
            {#each traffic.hours as hour}
              <div class="pb-2 text-center text-[9px] font-semibold text-[#778079]">{hourLabel(hour)}</div>
            {/each}

            {#each traffic.days as day}
              <div class="flex min-h-9 flex-col justify-center pr-2">
                <strong class={`text-[10px] ${day.enabled ? "text-[#414A44]" : "text-[#A0A5A2]"}`}>{day.label}</strong>
                <span class="mt-0.5 text-[8px] text-[#9AA19D]">{day.enabled ? `${day.start}–${day.end}` : "sem expediente"}</span>
              </div>
              {#each day.cells as cell}
                <div
                  class={`flex h-9 items-center justify-center rounded-lg border text-[9px] font-bold transition ${trafficIntensityClass(cell.count, traffic.maxCellCount, cell.available)}`}
      ...
      <div class="mt-4 flex flex-col justify-between gap-3 border-t border-[#EEF3EF] pt-4 sm:flex-row sm:items-center">
        <div class="flex flex-wrap items-center gap-2 text-[8px] text-[#7E8781]">
        ...
        <div class="text-[9px] text-[#727B75]">
      ...
      {#if !traffic.hoursConfigured}
        <p class="mt-3 rounded-xl bg-[#FFF8EC] px-3 py-2 text-[9px] leading-4 text-[#8A642B]">O horário de atendimento ainda não está marcado como configurado. Este mapa usa a grade padrão disponível nas configurações atuais.</p>
      {/if}
      <p class="mt-3 text-[8px] leading-4 text-[#9AA09C]">A classificação usa a timezone e o horário de atendimento configurados atualmente, aplicados ao histórico selecionado. Se o expediente foi alterado durante o período, os dados anteriores também são classificados pela configuração atual.</p>
    </div>
  </section>
