    ...
    {#if data.canManage}
      <section class="h-fit rounded-[22px] border border-[#E2E5ED] bg-white p-5 sm:p-6">
        ...
        <form method="POST" action="?/invite" class="mt-6 space-y-4">
          ...
          <label class="block"><span class="mb-1.5 block text-[11px] font-semibold text-[#4A5060]">Perfil base</span><select name="roleCode" value={values?.roleCode ?? "EMPLOYEE"} class="h-11 w-full rounded-xl border border-[#DDE1EA] bg-white px-3 text-[13px] outline-none transition focus:border-[#000A57] focus:ring-2 focus:ring-[#000A57]/10"><option value="EMPLOYEE">Funcionário</option><option value="VIEWER">Comercial · somente consulta</option>{#if data.canCreateAdmin}<option value="ADMIN">Admin</option>{/if}</select></label>
