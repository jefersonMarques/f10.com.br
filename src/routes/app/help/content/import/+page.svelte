...
<ApplicationContent width="narrow">
  <div class="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex flex-wrap gap-2">
      <a href="/templates/f10-help-import-v1-prompt.txt" class="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-3 text-[10px] font-semibold text-[#000A57]"><Download size={14}/>Prompt para IA</a>
      <a href="/templates/f10-help-import-v1.example.json" class="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-3 text-[10px] font-semibold text-[#000A57]"><Download size={14}/>Modelo JSON</a>
    </div>
  </div>

  <section class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
    <div class="rounded-2xl border border-[#E2E5ED] bg-white p-4"><FileJson2 size={20} class="text-[#000A57]"/><strong class="mt-3 block text-[13px]">JSON v1</strong><p class="mt-2 text-[10px] leading-5 text-[#858B99]">Ideal para texto, YouTube e mídias que continuarão por URL.</p></div>
    <div class="rounded-2xl border border-[#E2E5ED] bg-white p-4"><FileArchive size={20} class="text-[#000A57]"/><strong class="mt-3 block text-[13px]">ZIP + assets</strong><p class="mt-2 text-[10px] leading-5 text-[#858B99]">Use <code>manifest.json</code> e referências como <code>assets/tela.png</code>.</p></div>
    <div class="rounded-2xl border border-[#E2E5ED] bg-white p-4"><Bot size={20} class="text-[#EA6D0B]"/><strong class="mt-3 block text-[13px]">Pronto para IA</strong><p class="mt-2 text-[10px] leading-5 text-[#858B99]">Transcrições, resumos e conhecimento privado entram estruturados.</p></div>
    <div class="rounded-2xl border border-[#E2E5ED] bg-white p-4"><CheckCircle2 size={20} class="text-[#2F7045]"/><strong class="mt-3 block text-[13px]">Sempre rascunho</strong><p class="mt-2 text-[10px] leading-5 text-[#858B99]">Nenhuma importação ganha publicação automática.</p></div>
  </section>
  ...
  {#if form && "issues" in form && form.issues.length > 0}
    <section class="mt-4 rounded-[20px] border border-[#F0C8C8] bg-white p-5"><h2 class="text-[13px] font-semibold text-[#8D3333]">Problemas encontrados</h2><ul class="mt-3 space-y-2 text-[10px] leading-5 text-[#725454]">{#each form.issues as issue}<li class="rounded-xl bg-[#FFF7F7] px-3 py-2">{issue}</li>{/each}</ul></section>
  {/if}
