...
<ApplicationContent width="wide">
  <div class="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <span class="application-text-caption rounded-full bg-[#F3F4F7] px-3 py-1.5 font-semibold text-[#737989]">
      ...
      {#if data.content.hasPublishedVersion && data.content.status !== "published" && data.content.status !== "archived"}
        <span class="application-text-caption rounded-full bg-[#FFF4E9] px-3 py-1.5 font-bold text-[#B85408]">Há alterações não publicadas</span>
      {/if}
    </div>
  </div>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        {#if data.canEdit && !data.content.publishedAt}
          <form method="POST" action="/app/help/content?/discard" on:submit={(event) => { if (!confirm("Descartar este conteúdo definitivamente? Esta ação remove o rascunho e não pode ser desfeita.")) event.preventDefault(); }}>
            ...
            <button type="submit" class="application-text-caption inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[#F0C8C8] bg-white px-3.5 font-semibold text-[#9B2C2C] transition hover:bg-[#FFF5F5]"><Trash2 size={14}/>Descartar</button>
          </form>
        {:else if data.canPublish && data.content.publishedAt}
          <form method="POST" action="/app/help/content?/archive" on:submit={(event) => { if (!confirm("Arquivar este conteúdo? Ele deixará de aparecer na Central pública e deixará de ser usado pela IA. O histórico será mantido.")) event.preventDefault(); }}>
            <input type="hidden" name="contentId" value={data.content.id}/>
            <button type="submit" class="application-text-caption inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-3.5 font-semibold text-[#626979] transition hover:bg-[#F5F6F8]"><Archive size={14}/>Arquivar</button>
          </form>
        {/if}
        ...
        <div class="p-5 sm:p-6">
          <form method="POST" action="?/updateStep" class="grid gap-4 lg:grid-cols-2">
            ...
            <fieldset disabled={!data.canEdit} class="contents disabled:opacity-70">
              <label class="block"><span class="application-text-caption mb-1.5 block font-semibold text-[#555B6A]">Título do passo</span><input name="title" required maxlength="180" value={step.title} class="h-11 w-full rounded-xl border border-[#DDE1EA] px-3 text-[12px] outline-none focus:border-[#000A57]" /></label>
              <label class="block"><span class="application-text-caption mb-1.5 block font-semibold text-[#555B6A]">Descrição pública opcional</span><input name="description" maxlength="2000" value={step.description} class="h-11 w-full rounded-xl border border-[#DDE1EA] px-3 text-[12px] outline-none focus:border-[#000A57]" /></label>
              ...
              {#if data.canEdit}<div class="flex justify-end lg:col-span-2"><button type="submit" class="application-text-caption inline-flex min-h-9 items-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-4 font-semibold text-[#000A57]"><Save size={14} aria-hidden="true" />Salvar passo</button></div>{/if}
            </fieldset>
          </form>
                  ...
                  <div class={`rounded-2xl border p-4 sm:p-5 ${block.blockType === "video" ? "border-[#F0C8C8] bg-[#FFF8F7]" : "border-[#E3E6ED] bg-[#FAFAFC]"}`}>
                    ...
                    <form method="POST" action="?/updateBlock" class="mt-4 space-y-3">
                      ...
                      {:else if block.blockType === "image" && block.asset?.storageKey}
                        ...
                        <p class="application-text-meta text-[#7D8392]">Imagem gerenciada pela Biblioteca. Para reutilizar ou substituir, use a Biblioteca de arquivos.</p>
                      {:else if block.blockType === "image" || block.blockType === "video"}
                        ...
                        <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">URL da {block.blockType === "image" ? "imagem" : "mídia/vídeo"}</span><input name="sourceUrl" required value={block.asset?.sourceUrl ?? ""} class="h-10 w-full rounded-xl border border-[#DDE1EA] bg-white px-3 text-[11px]" /></label>
                        <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">Texto alternativo</span><input name="altText" maxlength="500" value={block.asset?.altText ?? ""} class="h-10 w-full rounded-xl border border-[#DDE1EA] bg-white px-3 text-[11px]" /></label>
                        <div class="grid gap-3 lg:grid-cols-2"><label class="block rounded-xl border border-[#D8DDF4] bg-[#F8F9FF] p-3"><span class="application-text-meta font-semibold text-[#000A57]">Transcrição / conteúdo da mídia para IA</span><textarea name="transcript" maxlength="100000" rows="4" class="application-text-caption mt-2 w-full resize-y rounded-lg border border-[#D8DDF4] bg-white px-3 py-2 leading-5">{block.asset?.transcript ?? ""}</textarea></label><label class="block rounded-xl border border-[#D8DDF4] bg-[#F8F9FF] p-3"><span class="application-text-meta font-semibold text-[#000A57]">Resumo adicional para IA</span><textarea name="aiSummary" maxlength="20000" rows="4" class="application-text-caption mt-2 w-full resize-y rounded-lg border border-[#D8DDF4] bg-white px-3 py-2 leading-5">{block.asset?.aiSummary ?? ""}</textarea></label></div>
                      {:else if block.blockType === "file" && block.asset}
                        <a href={`/api/app/help/assets/${block.asset.id}`} target="_blank" rel="noopener noreferrer" class="application-text-caption flex items-center justify-between gap-3 rounded-xl border border-[#DDE1EA] bg-white px-4 py-3 font-semibold text-[#000A57]"><span>{block.linkLabel || "Baixar arquivo"}</span><ExternalLink size={13}/></a>
                        <p class="application-text-meta text-[#7D8392]">Arquivo gerenciado pela Biblioteca. Remova este bloco para desvinculá-lo do passo; o arquivo original permanece disponível para reutilização.</p>
                      {:else if block.blockType === "link"}
                        <div class="grid gap-3 lg:grid-cols-2"><label><span class="application-text-meta mb-1 block font-semibold text-[#616777]">Texto do link</span><input name="linkLabel" required maxlength="240" value={block.linkLabel ?? ""} class="h-10 w-full rounded-xl border border-[#DDE1EA] bg-white px-3 text-[11px]" /></label><label><span class="application-text-meta mb-1 block font-semibold text-[#616777]">URL</span><input name="linkUrl" required value={block.linkUrl ?? ""} class="h-10 w-full rounded-xl border border-[#DDE1EA] bg-white px-3 text-[11px]" /></label></div>
                      {/if}

                      {#if data.canEdit && block.blockType !== "file" && !(block.blockType === "image" && block.asset?.storageKey)}<div class="flex justify-end"><button type="submit" class="application-text-caption inline-flex min-h-9 items-center gap-2 rounded-xl bg-white px-3 font-semibold text-[#000A57] shadow-sm ring-1 ring-[#DDE1EA]"><Save size={13} aria-hidden="true" />Salvar bloco</button></div>{/if}
                    </form>
                  </div>
                {/each}
              </div>
