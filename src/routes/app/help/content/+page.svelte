    ...
    {#if data.canEdit}
      <section class="h-fit rounded-[22px] border border-[#E2E5ED] bg-white p-5 sm:p-6">
        ...
        <form method="POST" action="?/create" class="mt-6 space-y-4">
          ...
          {#if data.categories.length === 0}<p class="rounded-xl border border-[#F0C8C8] bg-[#FFF5F5] px-3 py-2 text-[10px] text-[#9B2C2C]">Crie ao menos uma categoria ativa antes de criar artigos.</p>{/if}
