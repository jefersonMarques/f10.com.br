...
<ApplicationContent width="wide">
  ...
  <div class="mt-4 grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
    ...
    <section class="overflow-hidden rounded-[22px] border border-[#E2E5ED] bg-white">
      <header class="border-b border-[#EEF0F5] px-5 py-4"><h2 class="text-[14px] font-semibold">Arquivos ({data.assets.length})</h2><p class="application-text-meta mt-1 text-[#9297A5]">Arquivos podem ser reutilizados em passos sem duplicar armazenamento.</p></header>
        ...
        <div class="grid gap-px bg-[#EEF0F5] sm:grid-cols-2 lg:grid-cols-3">
          {#each data.assets as asset}
            <article class="bg-white p-4">
              <div class="flex h-24 items-center justify-center rounded-xl bg-[#F7F8FB]">{#if asset.assetType === "image"}<Image size={30} class="text-[#000A57]"/>{:else}<FileText size={30} class="text-[#EA6D0B]"/>{/if}</div>
              ...
              <div class="mt-3 flex items-center justify-between gap-2"><a href={`/api/app/help/assets/${asset.id}`} target="_blank" rel="noopener noreferrer" class="application-text-meta font-semibold text-[#000A57]">Abrir</a>{#if data.canEdit}<form method="POST" action="?/delete"><input type="hidden" name="assetId" value={asset.id}/><button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-[#A34242] hover:bg-[#FFF0F0]" aria-label="Excluir"><Trash2 size={14}/></button></form>{/if}</div>
            </article>
          {/each}
        </div>
      {/if}
