            ...
            <details class="overflow-hidden rounded-2xl border border-[#E3E6ED] bg-[#FAFAFC]" open={openStepId === step.id} on:toggle={(event) => handleStepToggle(event, step.id)}>
              ...
              <div class="border-t border-[#E7E9EF] bg-white p-4 sm:p-5">
                ...
                {#if interactionModeFor(step) === "action"}
                  <div class="mt-5 rounded-xl bg-[#F7F8FB] px-3 py-3 text-[10px] leading-5 text-[#6F7585]">Ao clicar em <strong>Não consegui</strong>, o participante escreve livremente o que aconteceu. O relato fica registrado com a pessoa, versão e microação para atendimento e melhoria da trilha.</div>
                {/if}
