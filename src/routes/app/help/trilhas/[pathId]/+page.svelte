...
<ApplicationContent width="wide">
  <div class="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span class={`application-text-caption rounded-full px-3 py-1.5 font-bold uppercase tracking-[0.08em] ${data.path.status === "published" ? "bg-[#EEF8F1] text-[#2F7045]" : data.path.status === "archived" ? "bg-[#F1F1F3] text-[#676D7D]" : "bg-[#EEF0FF] text-[#000A57]"}`}>{statusLabel(data.path.status)}</span>
    ...
    <div class="flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
      ...
      <div class="flex flex-wrap gap-2">
        {#if data.canPublish && data.path.status !== "published"}
          <form method="POST" action="?/publish"><button type="submit" class="application-text-caption inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#EA6D0B] px-4 font-semibold text-white"><GraduationCap size={15}/>Publicar nova versão</button></form>
        {/if}
        {#if data.canPublish && data.path.currentVersion > 0 && data.path.status !== "archived"}
          <form method="POST" action="?/archive" on:submit={(event) => { if (!confirm("Arquivar esta trilha? Novos convites serão bloqueados, mas participantes já iniciados continuam na versão recebida.")) event.preventDefault(); }}><button type="submit" class="application-text-caption inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#DDE1EA] bg-white px-3.5 font-semibold text-[#626979]"><Archive size={14}/>Arquivar</button></form>
        {/if}
      </div>
    </div>
  </section>
      ...
      <form method="POST" action="?/updatePath" class="rounded-[22px] border border-[#E2E5ED] bg-white p-5 sm:p-6">
        <fieldset disabled={!data.canEdit} class="disabled:opacity-70">
          ...
          <div class="mt-5 grid gap-4 lg:grid-cols-2">
            ...
            <label class="block lg:col-span-2"><span class="application-text-caption mb-1.5 block font-semibold text-[#555B6A]">Descrição interna</span><textarea name="description" rows="3" maxlength="1200" class="w-full rounded-xl border border-[#DDE1EA] px-3 py-2.5 text-[11px]">{data.path.description}</textarea></label>
            <label class="block lg:col-span-2"><span class="application-text-caption mb-1.5 block font-semibold text-[#555B6A]">Mensagem de entrada</span><textarea name="welcomeMessage" rows="3" maxlength="1200" class="w-full rounded-xl border border-[#DDE1EA] px-3 py-2.5 text-[11px]">{data.path.welcomeMessage}</textarea><span class="application-text-meta mt-1 block text-[#8B909D]">Evite “12 etapas”, “40 minutos” ou qualquer indicação de volume total.</span></label>
              ...
              <div class="border-t border-[#E7E9EF] bg-white p-4 sm:p-5">
                <form method="POST" action="?/updateStep" class="space-y-4">
                  ...
                  <fieldset disabled={!data.canEdit} class="space-y-4 disabled:opacity-70">
                    <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">Ação curta</span><input name="title" required maxlength="180" value={step.title} class="h-10 w-full rounded-xl border border-[#DDE1EA] px-3 text-[11px]"/></label>
                    <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">O que fazer agora</span><textarea name="instruction" required rows="4" maxlength="6000" class="w-full rounded-xl border border-[#DDE1EA] px-3 py-2.5 text-[11px] leading-5">{step.instruction}</textarea></label>
                    <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">O que deve aparecer quando terminar</span><textarea name="expectedResult" required rows="2" maxlength="3000" class="w-full rounded-xl border border-[#DDE1EA] px-3 py-2.5 text-[11px]">{step.expectedResult}</textarea></label>
                    <div class="grid gap-3 lg:grid-cols-[1fr_160px]"><label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">Mensagem de microvitória</span><input name="successMessage" maxlength="500" value={step.successMessage} class="h-10 w-full rounded-xl border border-[#DDE1EA] px-3 text-[11px]"/></label><label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#616777]">Segundos estimados</span><input name="estimatedSeconds" type="number" min="5" max="900" value={step.estimatedSeconds} class="h-10 w-full rounded-xl border border-[#DDE1EA] px-3 text-[11px]"/></label></div>
                ...
                <div class="mt-5 border-t border-[#EEF0F5] pt-5">
                  ...
                  <p class="application-text-meta mt-1 text-[#8B909D]">Adicione somente a mídia necessária para esta ação. Prints podem ser colados diretamente; vídeos são enviados como MP4 curto.</p>
                      ...
                      {#each step.media as media}
                        <div class="rounded-xl border border-[#E2E5ED] bg-[#FAFAFC] p-3">
                          ...
                          {:else if media.mediaType === "video" && media.sourceUrl}
                            <div class="flex min-h-28 items-center justify-center rounded-lg bg-[#EEF0FF] text-[#000A57]"><Video size={26}/></div><a href={media.sourceUrl} target="_blank" rel="noopener noreferrer" class="application-text-meta mt-2 inline-flex items-center gap-1 font-semibold text-[#000A57]">Vídeo legado<ExternalLink size={11}/></a>
                          {/if}
                          {#if data.canEdit}<form method="POST" action="?/deleteMedia" class="mt-2"><input type="hidden" name="stepId" value={step.id}/><input type="hidden" name="mediaId" value={media.id}/><button type="submit" class="application-text-meta inline-flex items-center gap-1 font-semibold text-[#9B2C2C]"><Trash2 size={11}/>Remover</button></form>{/if}
                        </div>
                      {/each}
                    </div>
                  {/if}
                ...
                <div class="mt-5 border-t border-[#EEF0F5] pt-5">
                  ...
                  <p class="application-text-meta mt-1 text-[#8B909D]">Cada motivo deve tentar recuperar o usuário antes de envolver atendimento humano.</p>
                  <div class="mt-3 space-y-2">
                    {#each step.failureReasons as reason}
                      <form method="POST" action="?/updateReason" class="rounded-xl border border-[#E2E5ED] bg-[#FAFAFC] p-3"><input type="hidden" name="stepId" value={step.id}/><input type="hidden" name="reasonId" value={reason.id}/><div class="grid gap-2 lg:grid-cols-[240px_1fr_auto]"><input name="label" required maxlength="180" value={reason.label} class="application-text-meta h-9 rounded-lg border border-[#DDE1EA] bg-white px-2"/><textarea name="recoveryMessage" rows="2" maxlength="4000" class="application-text-meta rounded-lg border border-[#DDE1EA] bg-white px-2 py-2 leading-4">{reason.recoveryMessage}</textarea>{#if data.canEdit}<button type="submit" class="application-text-meta h-9 rounded-lg border border-[#DDE1EA] bg-white px-3 font-semibold text-[#000A57]">Salvar</button>{/if}</div></form>
                    {/each}
                  </div>
                </div>
