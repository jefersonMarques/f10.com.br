...
import { addHelpHumanReviewCandidate } from "$lib/server/help/helpScreenshotReviewRepository";
...
export const POST: RequestHandler = async ({ cookies, params, request }) => {
  ...
  try {
    const candidate = await addHelpHumanReviewCandidate({
    ...
    return json({
      success: true,
      message: "Imagem adicionada como alternativa da revisão.",
      candidate,
    });
  } catch (cause) {
