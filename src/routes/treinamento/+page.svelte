...
{#if data.invitePreview && !data.state}
  <main class="flex min-h-[100dvh] items-center justify-center bg-[#F5F6FA] px-5 py-8 text-[#010D28]">
    <section class="w-full max-w-[820px] text-center">
      ...
      <p class="mx-auto mt-3 max-w-[620px] text-[13px] leading-7 text-[#656C7C]">Mantenha o F10 aberto. A tela principal mostra a referência visual e a guia flutuante conduz cada ação.</p>
      <form method="POST" action="?/acceptInvite" use:enhance={enhanceStart} class="mt-8"><button type="submit" class="training-start inline-flex min-h-14 items-center gap-2 rounded-full bg-[#F36B00] px-8 text-[12px] font-bold text-white shadow-[0_18px_36px_rgba(243,107,0,0.22)]">Começar trilha<ChevronRight size={17}/></button></form>
    </section>
  </main>
{:else if !data.state}
...
{:else if !data.state.session.startedAt}
  <main class="flex min-h-[100dvh] items-center justify-center bg-[#F5F6FA] px-5 py-8 text-[#010D28]">
    <section class="w-full max-w-[760px] text-center">
      ...
      <p class="mx-auto mt-3 max-w-[620px] text-[13px] leading-7 text-[#656C7C]">A referência visual fica na tela principal e a orientação abre em uma guia flutuante.</p>
