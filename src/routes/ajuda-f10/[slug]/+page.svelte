<script lang="ts">
  import { ArrowLeft, CheckCircle2, Download, ExternalLink, Info, PlayCircle, TriangleAlert } from "lucide-svelte";
...
<main class="min-h-screen bg-[#F7F8FB] text-[#10172A]">
  <div class="mx-auto max-w-[1080px] px-5 py-8 sm:px-8 sm:py-12">
    <a href="/ajuda-f10" class="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-[12px] font-semibold text-[#606777] transition hover:bg-white hover:text-[#000A57]"><ArrowLeft size={17}/>Central de Ajuda</a>
    ...
    <header class="mt-6 rounded-[28px] border border-[#E3E6EE] bg-white px-6 py-7 shadow-[0_14px_44px_rgba(1,13,40,0.05)] sm:px-9 sm:py-9">
      {#if data.content.category}<p class="text-[10px] font-bold uppercase tracking-[0.14em] text-[#EA6D0B]">{data.content.category}</p>{/if}
      <h1 class="mt-2 max-w-[860px] text-[30px] font-semibold tracking-[-0.04em] text-[#010D28] sm:text-[44px]">{data.content.title}</h1>
      {#if data.content.summary}<p class="mt-4 max-w-[820px] text-[14px] leading-7 text-[#6C7383] sm:text-[15px]">{data.content.summary}</p>{/if}
      <div class="mt-6 flex flex-wrap gap-2 text-[9px] font-semibold text-[#777E8E]"><span class="rounded-full bg-[#F4F5F8] px-3 py-1.5">{data.content.steps.length} {data.content.steps.length === 1 ? "passo" : "passos"}</span><span class="rounded-full bg-[#F4F5F8] px-3 py-1.5">Atualizado {new Intl.DateTimeFormat("pt-BR").format(new Date(data.content.publishedAt))}</span></div>
    </header>

    <div class="mt-7 space-y-5">
      {#each data.content.steps as step, index}
        <section class="overflow-hidden rounded-[26px] border border-[#E3E6EE] bg-white">
          <header class="flex items-start gap-4 border-b border-[#EEF0F5] px-5 py-5 sm:px-7">
            ...
            <div><h2 class="text-[18px] font-semibold tracking-[-0.02em] text-[#1F2638]">{step.title}</h2>{#if step.description}<p class="mt-1.5 text-[12px] leading-6 text-[#757C8D]">{step.description}</p>{/if}</div>
          </header>
            ...
            {#each step.blocks as block}
              ...
              {:else if block.blockType === "notice"}
                <div class={`flex items-start gap-3 rounded-2xl border px-4 py-3 ${block.noticeVariant === "warning" || block.noticeVariant === "danger" ? "border-[#F0D0C8] bg-[#FFF8F5] text-[#7D493D]" : "border-[#D8DEF2] bg-[#F8F9FF] text-[#4D587A]"}`}>
                  {#if block.noticeVariant === "warning" || block.noticeVariant === "danger"}<TriangleAlert size={17} class="mt-0.5 shrink-0"/>{:else}<Info size={17} class="mt-0.5 shrink-0"/>{/if}<p class="text-[12px] leading-6">{block.textContent}</p>
                </div>
              ...
              {:else if block.blockType === "video" && block.asset}
                {@const embed = youtubeEmbedUrl(block.asset.sourceUrl)}
                {#if embed}<div class="aspect-video overflow-hidden rounded-2xl bg-black"><iframe src={embed} title={block.asset.altText || step.title} class="h-full w-full" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>{:else if block.asset.sourceUrl}<a href={block.asset.sourceUrl} target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 rounded-2xl border border-[#E1E4EC] px-4 py-4 text-[11px] font-semibold text-[#000A57]"><PlayCircle size={18}/>Abrir vídeo<ExternalLink size={13}/></a>{/if}
    ...
    <section class="mt-7 flex items-center gap-3 rounded-[22px] border border-[#D8E9DE] bg-[#F4FBF6] px-5 py-4 text-[#356347]"><CheckCircle2 size={19}/><div><strong class="block text-[11px]">Passo a passo concluído</strong><span class="mt-1 block text-[9px] text-[#6E8C78]">Se ainda precisar de ajuda, utilize os canais de suporte da F10.</span></div></section>
  </div>
</main>
