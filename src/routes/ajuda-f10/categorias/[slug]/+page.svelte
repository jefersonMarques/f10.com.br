...
<svelte:head>
  ...
  <meta
    name="description"
    content={data.category.description || `Conteúdos de ajuda F10 sobre ${data.category.name}.`}
  />
</svelte:head>
...
<main class="min-h-screen bg-[#F7F8FB] text-[#10172A]">
  <section class="border-b border-[#E4E7EE] bg-[#010D28] text-white">
    <div class="mx-auto max-w-[980px] px-5 py-10 sm:px-8 sm:py-14">
      <a href="/ajuda-f10" class="inline-flex items-center gap-2 text-[11px] font-semibold text-white/65 hover:text-white">
        <ArrowLeft size={15}/>
        Central de Ajuda
      </a>

      <div class="mt-7 flex items-start gap-4">
        <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-[24px]">{data.category.icon || "📘"}</span>
        <div>
          <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#FF9A4B]">Categoria</p>
          <h1 class="mt-1 text-[30px] font-semibold tracking-[-0.04em] sm:text-[42px]">{data.category.name}</h1>
          {#if data.category.description}
            <p class="mt-3 max-w-[700px] text-[13px] leading-6 text-white/65">{data.category.description}</p>
          {/if}
        </div>
      </div>
      ...
      <form class="mt-8 max-w-[820px]" on:submit|preventDefault={askHelp}>
        <div class="flex items-end gap-2 rounded-[22px] border border-white/15 bg-white p-2 shadow-[0_18px_60px_rgba(0,0,0,0.16)] focus-within:border-[#EA6D0B] focus-within:ring-4 focus-within:ring-[#EA6D0B]/15">
          ...
          <textarea
            ...
            placeholder={data.helpPublicAi.available ? `Pergunte sobre ${data.category.name}` : "Pesquisa inteligente temporariamente indisponível"}
          ...
          ></textarea>
          <button type="submit" disabled={!data.helpPublicAi.available || loading || question.trim().length < 3} class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#EA6D0B] text-white transition hover:bg-[#D96208] disabled:cursor-not-allowed disabled:opacity-45" aria-label="Perguntar nesta categoria">
  ...
  <div class="mx-auto max-w-[980px] px-5 py-8 sm:px-8 sm:py-12">
        ...
        {:else}
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              <p class="whitespace-pre-line text-[13px] leading-6 text-[#343C4E]">{answer}</p>
              {#if target}
                <div class="mt-5 border-t border-[#EEF0F5] pt-4">
                  ...
                  {#if resolution === "answered"}<p class="mt-1 text-[10px] text-[#858B99]">Ver passo a passo completo</p>{/if}
                </div>
              {/if}
            </div>
          </div>
    ...
    <section class="mt-10">
      <div class="flex items-end justify-between gap-3">
        <div>
          <p class="text-[10px] font-bold uppercase tracking-[0.14em] text-[#EA6D0B]">Conteúdos publicados</p>
          <h2 class="mt-1 text-[24px] font-semibold tracking-[-0.035em] text-[#010D28]">{data.category.name}</h2>
        </div>
        <span class="text-[10px] text-[#8A909F]">{data.articles.length} {data.articles.length === 1 ? "artigo" : "artigos"}</span>
      </div>

      <div class="mt-6 grid gap-3 md:grid-cols-2">
        {#each data.articles as article}
          <a href={`/ajuda-f10/${encodeURIComponent(article.slug)}`} class="group rounded-[22px] border border-[#E2E5EC] bg-white p-5 shadow-[0_8px_24px_rgba(1,13,40,0.025)] transition hover:-translate-y-0.5 hover:border-[#CCD1DD]">
            <div class="flex items-start gap-3">
              <BookOpen size={17} class="mt-0.5 shrink-0 text-[#EA6D0B]"/>
              <div class="min-w-0">
                <h3 class="text-[13px] font-semibold leading-5 text-[#303746] group-hover:text-[#000A57]">{article.title}</h3>
                {#if article.summary}<p class="mt-2 line-clamp-3 text-[10px] leading-5 text-[#7D8493]">{article.summary}</p>{/if}
                <span class="mt-3 inline-flex items-center gap-1.5 text-[9px] font-semibold text-[#959BA8]">{article.stepCount} {article.stepCount === 1 ? "passo" : "passos"}<ArrowRight size={12}/></span>
              </div>
            </div>
          </a>
        {/each}
    ...
    <section class="mt-10 flex flex-col gap-5 rounded-[28px] border border-[#D9DDE7] bg-[#010D28] px-6 py-7 text-white sm:flex-row sm:items-center sm:justify-between sm:px-8">
      <div class="flex items-start gap-4">
        <span class="inline-flex h-11 min-w-11 items-center justify-center rounded-2xl bg-white/10 text-[#FF9A4B]"><LifeBuoy size={21}/></span>
        <div><h2 class="text-[18px] font-semibold">Ainda precisa de ajuda?</h2><p class="mt-1.5 max-w-[620px] text-[11px] leading-5 text-white/65">O atendimento F10 consulta esta mesma Base de Conhecimento e você pode pedir uma pessoa quando quiser.</p></div>
      </div>
