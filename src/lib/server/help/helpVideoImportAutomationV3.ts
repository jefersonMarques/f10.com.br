...
function articlePrompt(
  ...
  return [
    ...
    "Exemplo de screenshot planejado para preenchimento: { startSeconds: 92, endSeconds: 101, capture: 'after', target: 'Cadastro com o campo E-mail preenchido', altText: 'Campo E-mail preenchido no cadastro', assistantDescription: 'Etapa de preenchimento do campo E-mail' }.",
...
function candidateTimes(start: number, end: number): number[] {
  ...
  const fractions = [0.06, 0.24, 0.42, 0.6, 0.78, 0.96];
...
async function extractScreenshotCandidates(input: {
  ...
  for (const [candidateIndex, timeSeconds] of candidateTimes(window.start, window.end).entries()) {
    ...
    try {
      ...
      // Near EOF a single candidate can fail without invalidating the step.
    }
  }
  return candidates;
}
...
async function chooseStableCandidate(
  ...
  if (capture === "after") {
    const pair = stablePairs.at(-1) ?? [...pairs].sort((a, b) => b.score - a.score || b.index - a.index)[0];
    return pair ? candidates[pair.index + 1] ?? candidates.at(-1) ?? null : candidates.at(-1) ?? null;
  }

  const pair = stablePairs[0] ?? [...pairs].sort((a, b) => b.score - a.score || a.index - b.index)[0];
...
export async function generateHelpImportFromVideo(input: {
  ...
  try {
    ...
    await reportProgress(input.onProgress, {
      stage: "analyze",
      status: "active",
      label: "Capturando os screenshots nos tempos definidos",
    ...
    await reportProgress(input.onProgress, {
      stage: "analyze",
      status: "done",
      label: "Cortes e screenshots concluídos",
