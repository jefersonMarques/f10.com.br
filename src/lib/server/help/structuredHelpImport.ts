...
export type HelpImportBlock =
  ...
  | { type: "file"; url: string; label: string; extractedText?: string; assistantSummary?: string };
...
type ExistingImportedContent = { id: string; externalId: string | null; slug: string };
...
function parseStringArray(value: unknown, path: string, issues: string[], maxItems: number): string[] {
...
function parseCategories(value: unknown, path: string, issues: string[]): HelpImportCategory[] {
...
function parseFeaturedVideo(value: unknown, path: string, issues: string[]): HelpImportFeaturedVideo | undefined {
  ...
  if ((assistantSummary?.length ?? 0) > 20_000) issues.push(`${path}.assistantSummary: máximo de 20.000 caracteres.`);
...
function parseBlock(value: unknown, path: string, issues: string[]): HelpImportBlock | null {
  ...
  if (type === "image") {
    ...
    if ((assistantDescription?.length ?? 0) > 20_000) issues.push(`${path}.assistantDescription: máximo de 20.000 caracteres.`);
  ...
  if (type === "file") {
    ...
    if ((extractedText?.length ?? 0) > 200_000) issues.push(`${path}.extractedText: máximo de 200.000 caracteres.`);
    if ((assistantSummary?.length ?? 0) > 20_000) issues.push(`${path}.assistantSummary: máximo de 20.000 caracteres.`);
...
function parseStep(value: unknown, path: string, issues: string[]): HelpImportStep | null {
  ...
  if ((assistantKnowledge?.length ?? 0) > 20_000) issues.push(`${path}.assistantKnowledge: máximo de 20.000 caracteres.`);
...
function parseContent(value: unknown, index: number, issues: string[]): HelpImportContent | null {
  ...
  const searchAliases = parseStringArray(record.searchAliases ?? [], `${path}.searchAliases`, issues, MAX_SEARCH_ALIASES);
  const assistantKnowledge = optionalString(record, "assistantKnowledge");
  ...
  if (!externalId || externalId.length > 240) issues.push(`${path}.externalId: obrigatório e máximo de 240 caracteres.`);
  if (!title || title.length < 4 || title.length > 160) issues.push(`${path}.title: deve ter entre 4 e 160 caracteres.`);
  ...
  if ((quickGuide?.length ?? 0) > MAX_QUICK_GUIDE_CHARS) issues.push(`${path}.quickGuide: máximo de 12.000 caracteres.`);
  if ((assistantKnowledge?.length ?? 0) > 40_000) issues.push(`${path}.assistantKnowledge: máximo de 40.000 caracteres.`);
  if ((internalSupportNotes?.length ?? 0) > 40_000) issues.push(`${path}.internalSupportNotes: máximo de 40.000 caracteres.`);
  ...
  return { externalId, title, slug, summary, quickGuide, categories, searchAliases, assistantKnowledge, internalSupportNotes, featuredVideo, steps };
}
...
export function validateHelpImportJson(rawJson: string): HelpImportValidation {
  ...
  try {
    ...
    return { valid: false, source: "", contentCount: 0, stepCount: 0, blockCount: 0, issues: ["Arquivo JSON inválido."], parsed: null };
  }
  const root = asRecord(value);
  if (!root) {
    return { valid: false, source: "", contentCount: 0, stepCount: 0, blockCount: 0, issues: ["A raiz do arquivo deve ser um objeto JSON."], parsed: null };
  }
  ...
  if (!Array.isArray(contentsValue) || contentsValue.length === 0) issues.push("contents deve possuir ao menos um conteúdo.");
  else if (contentsValue.length > MAX_CONTENTS) issues.push(`Máximo de ${MAX_CONTENTS} conteúdos por arquivo.`);

  const contents = Array.isArray(contentsValue)
    ? contentsValue.map((content, index) => parseContent(content, index, issues)).filter((content): content is HelpImportContent => Boolean(content))
    : [];
  ...
  for (const [index, content] of contents.entries()) {
    ...
    if (!normalizedSlug) issues.push(`contents[${index}].slug: não foi possível gerar um endereço válido.`);
    else if (slugs.has(normalizedSlug)) issues.push(`contents[${index}].slug: duplicado no arquivo.`);
    slugs.add(normalizedSlug);
  }
  const stepCount = contents.reduce((total, content) => total + content.steps.length, 0);
  const blockCount = contents.reduce((contentTotal, content) => contentTotal + content.steps.reduce((stepTotal, step) => stepTotal + step.blocks.length, 0), 0);
  if (blockCount > MAX_TOTAL_BLOCKS) issues.push(`Máximo de ${MAX_TOTAL_BLOCKS} blocos por arquivo.`);
  const parsed: HelpImportFile | null = format === IMPORT_FORMAT && version === IMPORT_VERSION && source && Array.isArray(contentsValue) && contents.length === contentsValue.length && issues.length === 0
    ? { format: IMPORT_FORMAT, version: IMPORT_VERSION, source, contents }
    : null;
  return { valid: Boolean(parsed), source, contentCount: contents.length, stepCount, blockCount, issues, parsed };
}

function safeStorageName(value: string): string {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 120) || "screenshot.png";
}
function packageAssetReferences(content: HelpImportContent): string[] {
  return content.steps.flatMap((step) => step.blocks.flatMap((block) => {
...
async function uploadPackageAssets(
  ...
  try {
    ...
    await Promise.allSettled(Array.from(uploaded.values()).map((asset) => deleteAssetObject(asset.storageKey)));
    throw cause;
  }
}
...
export async function importStructuredHelpFile(
  ...
  const requestedCategorySlugs = Array.from(new Set(file.contents.flatMap((content) => content.categories.map((category) => category.slug))));
  const categoryRows = requestedCategorySlugs.length
    ? await db.select({ id: helpCategories.id, slug: helpCategories.slug }).from(helpCategories).where(and(inArray(helpCategories.slug, requestedCategorySlugs), eq(helpCategories.active, true)))
    : [];
  ...
  if (invalidCategories.length > 0) throw new Error(`IMPORT_CATEGORY_INVALID:${invalidCategories.join(",")}`);

  const externalIds = file.contents.map((content) => content.externalId);
  const existingRows = await db
    .select({ id: helpContents.id, externalId: helpContents.importExternalId, slug: helpContents.slug })
    .from(helpContents)
    .where(and(eq(helpContents.importSource, file.source), inArray(helpContents.importExternalId, externalIds)));
  const existingByExternalId = new Map(existingRows.filter((row): row is ExistingImportedContent => Boolean(row.externalId)).map((row) => [row.externalId, row]));
  const normalizedContents = file.contents.map((content) => {
    const existing = existingByExternalId.get(content.externalId) ?? null;
    return { ...content, contentId: existing?.id ?? randomUUID(), existing, normalizedSlug: normalizeHelpSlug(content.slug || content.title) };
  });
  if (normalizedContents.some((content) => !content.normalizedSlug)) throw new Error("IMPORT_INVALID_SLUG");

  const slugs = normalizedContents.map((content) => content.normalizedSlug);
  const slugRows = await db.select({ id: helpContents.id, slug: helpContents.slug }).from(helpContents).where(inArray(helpContents.slug, slugs));
  ...
  if (slugConflicts.length > 0) throw new Error(`IMPORT_SLUG_CONFLICT:${slugConflicts.map((item) => item.normalizedSlug).join(",")}`);

  const oldManagedAssets = existingRows.length
    ? await db.select({ contentId: helpAssets.contentId, storageKey: helpAssets.storageKey }).from(helpAssets).where(inArray(helpAssets.contentId, existingRows.map((row) => row.id)))
    : [];
  const oldStorageKeys = oldManagedAssets.map((asset) => asset.storageKey).filter((key): key is string => Boolean(key));
  const uploaded = await uploadPackageAssets(normalizedContents, packageAssets);
  const imported: Array<{ id: string; externalId: string; title: string; overwritten: boolean }> = [];

  try {
    await db.transaction(async (tx) => {
      for (const content of normalizedContents) {
        if (content.existing) {
          await tx.delete(helpContentFeaturedVideos).where(eq(helpContentFeaturedVideos.contentId, content.contentId));
          await tx.delete(helpContentCategories).where(eq(helpContentCategories.contentId, content.contentId));
          ...
          await tx.update(helpContents).set({
        ...
        await tx.insert(helpContentCategories).values(content.categories.map((category, categoryIndex) => ({
          contentId: content.contentId,
        ...
        if (content.featuredVideo) {
          const [videoAsset] = await tx.insert(helpAssets).values({
            contentId: content.contentId,
            assetType: "video",
          ...
          await tx.insert(helpContentFeaturedVideos).values({ contentId: content.contentId, assetId: videoAsset.id });
        }

        for (const [stepIndex, step] of content.steps.entries()) {
          const [createdStep] = await tx.insert(helpContentSteps).values({
            contentId: content.contentId,
            title: step.title,
          ...
          for (const [blockIndex, block] of step.blocks.entries()) {
            ...
            if (block.type === "image") {
              ...
              const [asset] = await tx.insert(helpAssets).values({
                contentId: content.contentId,
                assetType: "image",
        ...
        imported.push({ id: content.contentId, externalId: content.externalId, title: content.title, overwritten: Boolean(content.existing) });
      }
    });
  } catch (cause) {
    await Promise.allSettled(Array.from(uploaded.values()).map((asset) => deleteAssetObject(asset.storageKey)));
  ...
  return {
    ...
    blockCount: file.contents.reduce((contentTotal, content) => contentTotal + content.steps.reduce((stepTotal, step) => stepTotal + step.blocks.length, 0), 0),
    imported,
  };
}
