...
function getTokenKey(): Buffer {
  const secret = env.GOOGLE_CALENDAR_TOKEN_KEY?.trim() ?? "";
  if (secret.length < 32) {
    throw new Error("GOOGLE_CALENDAR_TOKEN_KEY must contain at least 32 characters.");
  }
...
export function decryptGoogleCalendarToken(value: string): string {
  ...
  if (version !== "v1" || !ivValue || !tagValue || !encryptedValue) {
    throw new Error("GOOGLE_CALENDAR_TOKEN_INVALID");
  }
