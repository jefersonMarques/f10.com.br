...
<div class={embedded ? "w-full" : "fixed bottom-5 left-5 z-[125] sm:bottom-6 sm:left-6"}>
  {#if !open}
    <button
      type="button"
      on:click={() => (open = true)}
  ...
  {:else}
    <aside class={embedded ? "w-full overflow-hidden rounded-[22px] border border-[#D9DDE8] bg-white shadow-[0_18px_50px_rgba(1,13,40,0.10)]" : "w-[min(390px,calc(100vw-32px))] overflow-hidden rounded-[24px] border border-[#D9DDE8] bg-white shadow-[0_24px_80px_rgba(1,13,40,0.24)]"}>
        ...
        {#if messages.length === 0}
          <div class="flex items-start gap-2.5">
            ...
            <div class="rounded-2xl rounded-tl-md bg-white px-4 py-3 shadow-[0_4px_18px_rgba(1,13,40,0.05)]">
              <p class="text-[11px] leading-5 text-[#424A5D]">{introText}</p>
            </div>
          </div>
        {:else}
          <div class="space-y-3">
