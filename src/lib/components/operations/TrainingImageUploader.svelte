...
>
  <div class="flex items-start gap-3">
    ...
    <div class="min-w-0 flex-1"><strong class="block text-[10px] font-semibold text-[#303645]">Adicionar print agora</strong><span class="mt-1 block text-[9px] leading-4 text-[#858B99]">Selecione, arraste ou ative este bloco e use <strong>Ctrl+V</strong>. A imagem também fica disponível na Biblioteca.</span></div>
  </div>
  <input bind:value={altText} maxlength="500" placeholder="Descrição opcional da imagem" class="mt-3 h-9 w-full rounded-lg border border-[#DDE1EA] bg-white px-3 text-[9px] outline-none focus:border-[#000A57]" />
  ...
  <button type="button" disabled={uploading} on:click={() => { activate(); fileInput?.click(); }} class="mt-2 inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg bg-[#000A57] px-3 text-[9px] font-semibold text-white disabled:cursor-wait disabled:opacity-60">
    {#if uploading}<UploadCloud size={13} class="animate-pulse"/>Enviando...{:else}<ClipboardPaste size={13}/>Selecionar ou colar print{/if}
  </button>
  {#if message}<p class="mt-2 text-[8px] font-medium text-[#257342]">{message}</p>{/if}
  {#if errorMessage}<p class="mt-2 text-[8px] font-medium text-[#A52A2A]">{errorMessage}</p>{/if}
</div>
