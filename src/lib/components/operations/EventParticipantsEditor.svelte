...
<div class="rounded-xl border border-[#E2E5ED] bg-white p-3">
  <div class="flex items-start gap-2">
    ...
    <div>
      <strong class="application-text-caption block font-semibold text-[#303747]">Participantes</strong>
      <span class="application-text-meta mt-0.5 block leading-4 text-[#858B98]">Equipe F10 por seleção; pessoas externas pelo e-mail. O Google enviará os convites.</span>
    </div>
  </div>

  {#if users.length > 1}
    <div class="mt-3 rounded-lg border border-[#E6E8EE] bg-[#FAFAFC] p-2.5">
      <label class="block"><span class="application-text-meta mb-1 block font-semibold text-[#707786]">Equipe F10</span><input type="search" bind:value={userSearch} placeholder="Buscar nome ou e-mail" class="application-text-caption h-9 w-full rounded-lg border border-[#DDE1EA] bg-white px-2.5 outline-none focus:border-[#000A57]"/></label>
      <div class="mt-2 max-h-36 space-y-1 overflow-y-auto pr-1">
        {#each filteredUsers as user}
          ...
          <div class="flex items-center justify-between gap-2 rounded-lg bg-white px-2.5 py-2">
            <label class="flex min-w-0 flex-1 cursor-pointer items-center gap-2"><input type="checkbox" checked={selected} on:change={(event) => toggleInternal(user, (event.currentTarget as HTMLInputElement).checked)}/><span class="min-w-0"><strong class="application-text-meta block truncate font-semibold text-[#424957]">{user.name}</strong><span class="application-text-meta block truncate text-[#9297A4]">{user.email}</span></span></label>
            {#if selected}
              <div class="flex shrink-0 flex-wrap items-center justify-end gap-2">
                <span class={`application-text-meta font-semibold ${responseClass(attendee?.responseStatus)}`}>{responseLabel(attendee?.responseStatus)}</span>
                {#if !allDay}
                  <span class={`application-text-meta font-semibold ${availability?.conflicts.length ? "text-[#A9510D]" : availability ? "text-[#2F7045]" : "text-[#8D93A0]"}`}>{availabilityLoading && !availability ? "Verificando..." : availability ? conflictLabel(availability) : ""}</span>
                {/if}
                <label class="application-text-meta flex items-center gap-1 text-[#7D8391]"><input type="checkbox" checked={Boolean(attendee?.optional)} on:change={(event) => setInternalOptional(user.id, (event.currentTarget as HTMLInputElement).checked)}/>Opcional</label>
              </div>
            {/if}
          </div>
        {/each}
  ...
  <div class="mt-3">
    <span class="application-text-meta mb-1 block font-semibold text-[#707786]">Participantes externos</span>
    <div class="flex gap-2"><input type="email" bind:value={externalEmail} on:keydown={(event) => { if (event.key === "Enter") { event.preventDefault(); addExternal(); } }} placeholder="cliente@empresa.com" class="application-text-caption h-9 min-w-0 flex-1 rounded-lg border border-[#DDE1EA] px-2.5 outline-none focus:border-[#000A57]"/><button type="button" on:click={addExternal} class="application-text-meta inline-flex h-9 items-center gap-1 rounded-lg border border-[#D6DAE3] bg-[#FAFAFC] px-3 font-semibold text-[#000A57]"><Plus size={12}/>Adicionar</button></div>
    {#if externalError}<p class="application-text-meta mt-1 font-medium text-[#A52A2A]">{externalError}</p>{/if}
      ...
      <div class="mt-2 space-y-1.5">
        {#each externalAttendees as attendee}
          <div class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[#E7E9EF] bg-[#FAFAFC] px-2.5 py-2">
            <div class="min-w-0"><span class="application-text-meta block truncate font-medium text-[#4A5060]">{attendee.email}</span>{#if attendee.responseStatus}<span class={`application-text-meta ${responseClass(attendee.responseStatus)}`}>{responseLabel(attendee.responseStatus)}</span>{/if}</div>
            <div class="flex items-center gap-2"><label class="application-text-meta flex items-center gap-1 text-[#7D8391]"><input type="checkbox" checked={Boolean(attendee.optional)} on:change={(event) => setOptional(attendee.email, (event.currentTarget as HTMLInputElement).checked)}/>Opcional</label><button type="button" on:click={() => removeAttendee(attendee.email)} class="flex h-7 w-7 items-center justify-center rounded-md text-[#8C919E] hover:bg-white" aria-label="Remover participante"><X size={12}/></button></div>
          </div>
        {/each}
      </div>
    {/if}
  ...
  {#if !allDay && internalUserIds.length > 0}
    <div class="mt-3 border-t border-[#ECEEF3] pt-2">
      {#if availabilityLoading}<p class="application-text-meta inline-flex items-center gap-1.5 text-[#7D8391]"><Clock3 size={11}/>Verificando disponibilidade da equipe...</p>{/if}
      {#if availabilityError}<p class="application-text-meta inline-flex items-center gap-1.5 font-medium text-[#A9510D]"><CircleAlert size={11}/>{availabilityError}</p>{/if}
      {#if availabilityResults.some((result) => result.conflicts.length > 0)}<p class="application-text-meta mt-1 inline-flex items-center gap-1.5 font-semibold text-[#A9510D]"><CircleAlert size={11}/>Há conflito de horário. O F10 avisa, mas não bloqueia a criação.</p>{/if}
      {#if availabilityResults.some((result) => result.coverage === "f10-only")}<p class="application-text-meta mt-1 leading-3 text-[#9297A4]">Para usuários sem agenda Google disponível, a checagem considera somente compromissos temporizados conhecidos pelo F10.</p>{/if}
    </div>
  {:else if allDay && internalUserIds.length > 0}
    <p class="application-text-meta mt-3 border-t border-[#ECEEF3] pt-2 text-[#9297A4]">Disponibilidade automática é verificada apenas para eventos com horário.</p>
  {/if}
</div>
